...
import { StatusAPI } from "../../backGroundServices/webApi/WebApi";

//TODO: apply device type check and show user in GUI like XAOIMI
//TODO: apply firmware and device compatibality check
//TODO: notify user for available upgrade
//TODO: implement react-native-wifi-reborn for auto connect
...
PairIos1 = props => {
  ...
  useEffect(() => {
    ...
    return () => {
      {
        debug && console.log("PairIOS1 USE EFFECT_Cleanup");
      }
  ...
  onSettings = () => {
    //FIXME: ios WiFi settings not opening
    DeviceSettings.wifi();
    ///props.navigation.navigate("PairIos2")
  };

  Next = () => {
    //TODO: send status response of the device along with props
    let wifiName = {};
    ...
    props.navigation.replace("PairingForm", {
      otherParam: {
        wifiList: wifiList,
        selectedDevice: wifiName,
        currDeviceStatus: currDeviceStatus
      }
    });
  };

  netFunction = info => {
    //FIXME: apply check for android only<--this functionto work for android only-->
    if (info.type == "wifi") {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={{ alignSelf: "center" }}>
          ...
          >
            ...
            >
              ...
              <Light>
                {" "}
                Connect to a HUElite Device WiFi "<Bold>HUE_CLXX_XX:XX</Bold>"
                with password <Bold>12345678</Bold>
              </Light>
            </Bold>
            <Bold
              style={{ color: "#aaa", textAlign: "center", paddingTop: 10 }}
            >
              3.
              <Light>
                {" "}
      ...
      >
        <Button style={[styles.Button, styles.settings]}>
          <Text
...
const Light = props => {
  return (
    <Text style={[props.style, { fontWeight: "400" }]}>{props.children}</Text>
  );
};
