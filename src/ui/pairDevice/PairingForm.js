...
import { getAvailableDevice } from "../wifiSearch/WifiScan"
    ...
    componentDidMount() {
        ...
        this.unsubscribe = NetInfo.addEventListener(info => {
            if (info.isConnected && info.type === "wifi") {
                ...
                console.log(":::::::" + info.details.ssid)
                if (SSID == info.details.ssid) this.setState({ currentWifiStatus: "connected" })
                else this.setState({ currentWifiStatus: "none" })
            }
        })
    }
                    ...
                    if (data.includes("Attempting CONN")) {
                        ...
                        if (IP && IP.length) {
                            ...
                            if (saveWifiRes === "RES-551") {
                                let newDevice = [
                                    createNewDevice({ BSSID: BSSID.toUpperCase(), SSID, IP }),
                                ]
    ...
    onSkip = async () => {
        let { deviceMAC, deviceName } = this.state,
            ...
            newDevice = createNewDevice({
                BSSID: deviceMAC.toUpperCase(),
                SSID: deviceName,
                IP: "192.168.4.1",
            }),
            self = this
        deviceArr.push(newDevice)
        insertDevices(deviceArr) //check
        let dbRes = await getDeviceListFromDb(),
            newDeviceList = dbRes.data
        if (newDeviceList.length) {
                ...
                <ScrollView style={{ height: "70%" }}>
                    <View style={styles.form}>
                        ...
                        {anyEmptyFld && !cnfPass.length ? (
                            <Text style={{ color: "red" }}>Please enter confirm password</Text>
                        ) : null}
                    ...
                    <View style={styles.footer}>
                        <TouchableOpacity onPress={() => this.onSkip()}>
                            <Text style={[styles.displayTxt, { fontSize: FONT_SIZE.FONT_16 }]}>
                                Skip
                            </Text>
                        </TouchableOpacity>
